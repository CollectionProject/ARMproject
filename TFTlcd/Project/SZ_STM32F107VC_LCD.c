...
//    Devicestr[18] = ((DeviceIdCode >> 0) & 0xF) + 0x30;
    ...
    else if(DeviceIdCode==0x8989)
    {
        ...
        LCD_WriteReg(0x0003,0xAEA4);    //0xA8A4
        LCD_WriteReg(0x000C,0x0004);    
        LCD_WriteReg(0x000D,0x000C);    
        LCD_WriteReg(0x000E,0x2800);    
        LCD_WriteReg(0x001E,0x00B5);    
			
        //LCD_WriteReg(0x0001,0x393F);    //横屏正
 			  LCD_WriteReg(0x0001,0x7b3F);      //横屏反
			
        LCD_WriteReg(0x0002,0x0600);     //LCD Driving Waveform control
        LCD_WriteReg(0x0010,0x0000);    
        LCD_WriteReg(0x0011,0x6078);    //6078  0x4030    //定义数据格式  16位色 0x6070    横屏 0x6058
        LCD_WriteReg(0x0005,0x0000);    
        LCD_WriteReg(0x0006,0x0000);    
        LCD_WriteReg(0x0016,0xEF1C);    
        //LCD_WriteReg(0x0017,0x0003);    
        LCD_WriteReg(0x0007,0x0033);    //0x0233       
