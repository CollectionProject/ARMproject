...
static void WiFi_LowLevel_GPIOInit(void)
{
  ...
  // 使能WiFi模块的复位信号
  gpio.GPIO_Mode = GPIO_Mode_Out_PP; // PB12设为推挽输出, 并立即输出低电平
  gpio.GPIO_Pin = GPIO_Pin_12;
  gpio.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(GPIOB, &gpio);
  
  delay(100); // 延时一段时间, 使WiFi模块能够正确复位
  GPIO_WriteBit(GPIOB, GPIO_Pin_12, Bit_SET); // 撤销复位信号
...
#ifdef WIFI_USEDMA
  while (DMA_GetFlagStatus(DMA2_FLAG_TC4) == RESET) // 等待DMA读取数据完毕
  {
    if (SDIO_GetFlagStatus(SDIO_FLAG_RXACT) == RESET || SDIO_GetFlagStatus(SDIO_FLAG_RXOVERR) == SET)
      break; // 如果出现错误, 则退出循环
  }
  DMA_ClearFlag(DMA2_FLAG_TC4); // 清除DMA传输完成标志位
...
#ifdef WIFI_USEDMA
  while (DMA_GetFlagStatus(DMA2_FLAG_TC4) == RESET) // 等待DMA送入数据完毕
  {
