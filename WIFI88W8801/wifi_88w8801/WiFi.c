...
static void WiFi_Associate_Callback(void *arg, void *data, WiFi_Status status)
{
  ...
  switch (WiFi_GetCommandCode(data))
  {
    case CMD_802_11_SCAN:
      ...
      WiFi_SendCommand(CMD_802_11_ASSOCIATE, wifi_buffer_command, cmd_size, WiFi_Associate_Callback, arg, WIFI_TIMEOUT_SCAN, WIFI_DEFAULT_MAXRETRY);
      // 保留arg内存, 等关联成功后再释放
      break;
...
static void WiFi_EventProcess(const WiFi_Event *event)
{
  printf("[Event %d] size=%d\n", event->event_id, event->header.length);
...
{
  WiFi_SendCommand(CMD_TX_PKT_STATS, wifi_buffer_command, sizeof(WiFi_CommandHeader), WiFi_PrintTxPacketStatus_Callback, NULL, WIFI_DEFAULT_TIMEOUT_CMDRESP, WIFI_DEFAULT_MAXRETRY);
}
...
static void WiFi_PrintTxPacketStatus_Callback(void *arg, void *data, WiFi_Status status)
{
  ...
  if (status == WIFI_STATUS_OK)
  {
    ...
    {
      if (wifi_ratelist[i] != 0.0f && !isempty(&entries[i], sizeof(WiFi_TxPktStatEntry)))
      {
        none = 0;
...
void WiFi_Scan(WiFi_Callback callback, void *arg)
{
  ...
  WiFi_SendCommand(CMD_802_11_SCAN, wifi_buffer_command, sizeof(WiFi_CmdRequest_Scan) + TLV_STRUCTLEN(*chanlist), WiFi_Scan_Callback, p, WIFI_TIMEOUT_SCAN, WIFI_DEFAULT_MAXRETRY);
}
...
static void WiFi_Scan_Callback(void *arg, void *data, WiFi_Status status)
{
  ...
  if (j < 14)
  {
    ...
    WiFi_SendCommand(CMD_802_11_SCAN, wifi_buffer_command, sizeof(WiFi_CmdRequest_Scan) + TLV_STRUCTLEN(*chanlist), WiFi_Scan_Callback, arg, WIFI_TIMEOUT_SCAN, WIFI_DEFAULT_MAXRETRY);
  }
  else
    n = 0;
...
void WiFi_ScanSSID(const char *ssid, WiFi_SSIDInfo *info, WiFi_Callback callback, void *arg)
{
  ...
  WiFi_SendCommand(CMD_802_11_SCAN, wifi_buffer_command, ((uint8_t *)chan_list - wifi_buffer_command) + TLV_STRUCTLEN(*chan_list), WiFi_ScanSSID_Callback, p, WIFI_TIMEOUT_SCAN, WIFI_DEFAULT_MAXRETRY);
}
...
// 返回值表示发送数据所用的通道号
int8_t WiFi_SendPacket(void *data, uint16_t size, WiFi_Callback callback, void *arg, uint32_t timeout)
{
...
void WiFi_SetWEPEx(const WiFi_WEPKey *key, WiFi_Callback callback, void *arg)
{
  ...
  MrvlIETypes_KeyParamSet_v2_t *keyparamset = (MrvlIETypes_KeyParamSet_v2_t *)&cmd->keys; // 设置密钥时必须使用V2版本的TLV参数
  ...
  WiFi_SendCommand(CMD_802_11_KEY_MATERIAL, wifi_buffer_command, (uint8_t *)plast - wifi_buffer_command, callback, arg, WIFI_DEFAULT_TIMEOUT_CMDRESP, WIFI_DEFAULT_MAXRETRY);
}
...
static void WiFi_SetWEPEx_Callback(void *arg, void *data, WiFi_Status status)
{
  // 移除旧密钥
  WiFi_Cmd_KeyMaterial *cmd = (WiFi_Cmd_KeyMaterial *)wifi_buffer_command;
  MrvlIETypes_KeyParamSet_v2_t *keyparamset = (MrvlIETypes_KeyParamSet_v2_t *)&cmd->keys;
  ...
  *plast += 10;
  WiFi_SendCommand(CMD_802_11_KEY_MATERIAL, wifi_buffer_command, TLV_NEXT(keyparamset) - wifi_buffer_command, WiFi_SetWEPEx_Callback, arg, WIFI_DEFAULT_TIMEOUT_CMDRESP, WIFI_DEFAULT_MAXRETRY);
}
...
void WiFi_SetWPA(const char *password, WiFi_Callback callback, void *arg)
{
  ...
  WiFi_SendCommand(CMD_SUPPLICANT_PMK, cmd, TLV_NEXT(passphrase) - wifi_buffer_command, callback, arg, WIFI_TIMEOUT_SCAN, WIFI_DEFAULT_MAXRETRY);
}
...
void WiFi_ShowKeyMaterials(void)
{
  ...
  WiFi_SendCommand(CMD_802_11_KEY_MATERIAL, wifi_buffer_command, cmd->keys.key - wifi_buffer_command, WiFi_ShowKeyMaterials_Callback, NULL, WIFI_DEFAULT_TIMEOUT_CMDRESP, WIFI_DEFAULT_MAXRETRY);
