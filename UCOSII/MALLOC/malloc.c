...
//size:内存大小(字节)
//返回值:分配到的内存首地址.
void *mymalloc(u8 type, u32 size)  
{  
    u32 offset;  									      
...
//size:要分配的内存大小(字节)
//返回值:新分配到的内存首地址.
void *myrealloc(u8 type, void *ptr,u32 size)  
{  
    u32 offset;  
    offset=mem_malloc(size);  
    if(offset==0XFFFFFFFF)return NULL;     
